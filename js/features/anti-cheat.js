 ...
 * Prevents cheating during MCQ tests
 */
    ...
    const AntiCheat = {
        ...
        detectTestMode() {
            const checkMode = () => {
                ...
                const hasQuizElements = document.querySelector('.quiz-container, .question-container, [class*="quiz"], [class*="test"], [class*="mcq"]');
                const urlHasQuiz = url.includes('quiz') || url.includes('test') || url.includes('battle');
                
                if ((hasQuizElements || urlHasQuiz) && !this.isActive) {
                    setTimeout(() => this.activate(), 1000);
                } else if (!hasQuizElements && !urlHasQuiz && this.isActive) {
                    this.deactivate();
                }
            };
